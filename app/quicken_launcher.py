...
def open_in_quicken(
    ...
    """
    Open *qfx_path* in Quicken.

    - If Quicken is not running it is launched with the QFX file.
    - If Quicken is already running the new process call is intercepted by
      Quicken's single-instance handler and the import dialog opens in the
      existing window.
    ...
    if already_running:
        _log("Quicken already running — passing QFX to open instance...")
    else:
        _log("Launching Quicken...")

    try:
        subprocess.Popen([qw_exe, "-X", qfx_path])
